...
# Vendor the runtime dependencies:
#   @agentclientprotocol/sdk (and its peer zod)
#   web-tree-sitter  (JS binding + core wasm; required for the tree-sitter bash scanner)
#   tree-sitter-bash (grammar wasm; fallback when kilo's bundled grammar is unavailable)
cp -R node_modules/@agentclientprotocol "$RUNTIME_DIR/node_modules/"
cp -R node_modules/zod "$RUNTIME_DIR/node_modules/"
cp -R node_modules/jsonc-parser "$RUNTIME_DIR/node_modules/"
cp -R node_modules/web-tree-sitter "$RUNTIME_DIR/node_modules/"
cp -R node_modules/tree-sitter-bash "$RUNTIME_DIR/node_modules/"
