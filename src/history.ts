...
// Update a most-recently-used list: remove any existing occurrence of `value`,
// move it to the front, and cap the list to `cap` entries (most-recent first).
// Pure (no I/O) so it can be unit-tested like trimHistoryEntries. Used to
// track recently-used models per project (see index.ts recent_models file).
export function updateRecentList(
  ...
  const v = value.trim();
  if (!v) return existing.slice();
  const next = existing.filter((x) => x !== v);
  next.unshift(v);
  if (cap > 0 && next.length > cap) next.length = cap;
  return next;
}
