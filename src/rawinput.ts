...
export class RawInput {
  ...
  private doTab(): void {
    ...
    const line = this.lines[this.cursorRow];
    const [hits, matched] = this.completer(line, this.cursorCol);
    ...
    this.renderCompletions(hits);
  }

  // Render completion variants below the input. Reserves at most 2 screen rows
  // for the matches (laid out in columns to use width), and when there are
  // more matches than fit, a "... (N more)" indicator is shown instead of
  // scrolling the screen with a long list. The input is then redrawn below.
  private renderCompletions(hits: string[]): void {
    ...
    const MAX_ROWS = 2;
    const cap = perRow * MAX_ROWS;
    const show = hits.slice(0, cap);
    const more = hits.length - show.length;

    let buf = "\r\n";
    for (let i = 0; i < show.length; i++) {
      const pad = Math.max(0, colW - widths[i]);
      buf += `${hits[i]}${" ".repeat(pad)}`;
      if ((i + 1) % perRow === 0) buf += "\r\n";
    }
    if (show.length % perRow !== 0) buf += "\r\n";
    if (more > 0) buf += `${dim}... (${more} more)${reset}\r\n`;
