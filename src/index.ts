...
import {
  ...
  matchModelWords,
  matchModelPaths,
  matchModels as matchModelsPure,
  exactModelMatch,
  formatEffortDisplay,
  type ModelOption,
} from "./modelSearch.js";
...
import { handleKiloBalance, handleKiloSessions, handleKiloGatewayRequests, readKiloCachedTokens } from "./kiloBalance.js";
import { handleOpenRouterBalance } from "./openrouterBalance.js";
import { parseSlashCommandInvocation } from "./slashCommand.js";
import {
  serializeHistory,
...
// Most-recently-used models: stored in kilo's own state file
// (~/.local/state/kilo/model.json) under its `recent` array, NOT in a separate
// per-project file. Up to RECENT_MODELS_STORE_CAP are kept; only the newest 2
// not already in the favourites list are surfaced in /model (greyed). The
// legacy per-project `.kilo/recent_models` file is migrated once (below) then
// ignored.
const LEGACY_RECENT_MODELS_FILE = join(HISTORY_DIR, "recent_models");
const RECENT_MODELS_STORE_CAP = 5;
const RECENT_MODELS_SHOW = 2;

// Kilo's own model-state file (same one the TUI/CLI read+write): the saved
// default thinking level per model (`variant` map) and the global recently-
// used models list (`recent` array of {providerID, modelID}). We read/write
// this so /thinking's choice sticks across restarts and recents are shared
// with kilo. Resolution: $XDG_STATE_HOME/kilo/model.json, else
// ~/.local/state/kilo/model.json (matches kilo's Global.Path.state).
const MODEL_STATE_DIR = join(
  process.env.XDG_STATE_HOME || join(os.homedir(), ".local/state"),
  "kilo",
);
const MODEL_STATE_FILE = join(MODEL_STATE_DIR, "model.json");
...
// Read kilo's model-state file (model.json). Best-effort: any read/parse
// failure yields an empty state. Shape mirrors kilo's KilocodeModelState.State:
//   { model: Record<role, {providerID, modelID}>, recent: [{providerID, modelID}],
//     favorite: [{providerID, modelID}], variant: Record<"<provider>/<model>", id> }
...
function readModelState(): ModelState {
  try {
    if (!fs.existsSync(MODEL_STATE_FILE)) return {};
    const data = JSON.parse(fs.readFileSync(MODEL_STATE_FILE, "utf-8"));
    if (data && typeof data === "object") return data as ModelState;
    return {};
  } catch {
    return {};
  }
...
// Atomically (best-effort) write the model-state file, preserving any keys we
// don't touch. Creates the parent dir if missing.
function writeModelState(next: ModelState): void {
  try {
    fs.mkdirSync(MODEL_STATE_DIR, { recursive: true });
  } catch {
    /* read-only parent; write below will fail and be reported */
  }
  try {
    fs.writeFileSync(MODEL_STATE_FILE, JSON.stringify(next, null, 2) + "\n", "utf-8");
  } catch (err) {
    console.error(`⚠️  Could not write ${MODEL_STATE_FILE}: ${(err as Error).message}`);
  }
}

// One-time migration of the legacy per-project `.kilo/recent_models` file into
// model.json's `recent` array. Runs at most once: if model.json already has a
// non-empty `recent`, the legacy file is ignored (kilo's own recents win).
// After a successful import the legacy file is deleted so it stops shadowing.
let legacyRecentMigrated = false;
function migrateLegacyRecentModels(): void {
  if (legacyRecentMigrated) return;
  legacyRecentMigrated = true;
  try {
    if (!fs.existsSync(LEGACY_RECENT_MODELS_FILE)) return;
  } catch {
    return;
  }
  const state = readModelState();
  const existing = Array.isArray(state.recent) ? state.recent : [];
  if (existing.length > 0) {
    // kilo's recents already present — drop the legacy file so it can't
    // shadow on future reads, but don't import.
    try {
      fs.unlinkSync(LEGACY_RECENT_MODELS_FILE);
    } catch {
      /* ignore */
    }
    return;
  ...
  let legacy: string[] = [];
  try {
    const content = fs.readFileSync(LEGACY_RECENT_MODELS_FILE, "utf-8");
  ...
  // Convert "<provider>/<model>" ids (split on the first "/") into model.json's
  // {providerID, modelID} refs. modelID itself may contain "/" (e.g. "z-ai/glm-5.2").
  const refs: ModelRef[] = [];
  ...
  state.recent = refs.slice(0, RECENT_MODELS_STORE_CAP);
  writeModelState(state);
  try {
    fs.unlinkSync(LEGACY_RECENT_MODELS_FILE);
  } catch {
    /* ignore — best-effort; the migrated state is what matters */
  }
}

// Read the recently-used models list (most-recent first) as "<provider>/<model>"
// ids — the format /model's favValues uses. Backed by model.json's `recent`
// array. Best-effort: any read/parse failure yields an empty list.
function readRecentModels(): string[] {
  ...
  try {
    ...
    for (const r of recent) {
      if (!r || typeof r.providerID !== "string" || typeof r.modelID !== "string") continue;
...
// Record a model as just-used: move it to the front of the recent list and
// persist (best-effort) to model.json. Called both on explicit /model switches
// and on session start/resume so a model merely started with also counts as
// "used". `value` is a "<provider>/<model>" id.
function recordRecentModel(value: string): void {
  const v = value?.trim();
  if (!v) return;
  try {
    const state = readModelState();
    ...
    const existingIds = existing
      .filter((r) => r && typeof r.providerID === "string" && typeof r.modelID === "string")
      .map((r) => `${r.providerID}/${r.modelID}`);
    const nextIds = updateRecentList(existingIds, v, RECENT_MODELS_STORE_CAP);
    state.recent = nextIds.map((id) => {
      const sep = id.indexOf("/");
      return sep <= 0
        ? { providerID: id, modelID: "" }
        : { providerID: id.slice(0, sep), modelID: id.slice(sep + 1) };
    });
    writeModelState(state);
  } catch {
    /* ignore write failures — best-effort recency tracking */
  }
}

// Save a model's default thinking level (variant) to model.json's `variant`
// map, mirroring kilo's own saveVariant (../kilocode-src
// packages/opencode/src/cli/cmd/run/variant.shared.ts). `modelId` is a
// "<provider>/<model>" id; `variant` is the variant id, or null to clear it
// (which makes the model fall back to its base level). Best-effort.
function saveModelVariant(modelId: string, variant: string | null): void {
  try {
    const state = readModelState();
    const variantMap = (state.variant && typeof state.variant === "object") ? { ...state.variant } : {};
    if (variant) {
      variantMap[modelId] = variant;
    } else {
      delete variantMap[modelId];
    }
    state.variant = variantMap;
    writeModelState(state);
  } catch {
    /* ignore — best-effort persistence; the live switch already took effect */
  }
}
...
// Human-readable titles for main sessions, keyed by session id. Populated from
// session/list (on resume and refreshed before /status) so the status view can
// print the session name. Summarizer sessions have no meaningful title.
...
// cached for hot-reload). The agent is forced (via KILO_CONFIG_CONTENT's
// ASK_PERMISSION override) to send `requestPermission` for every real tool
// call, so the client is the authoritative decision-maker and re-evaluates
// these *real* user rules on every request — global (lowest) then project
// files in precedence order (later wins, matching kilo's merge). This lets
// mid-session edits to .kilo/kilo.jsonc (e.g. a local deny overriding a
// global allow) take effect without a restart.
...
// project files, local overwrites global — computed via
// computeMergedPermissionObject). Memory is a PURE CACHE of the
...
// Tools the client forces the agent to always ask about (via
// KILO_CONFIG_CONTENT, merged last → overrides the user's real allow/deny
// rules). This makes the agent send `requestPermission` for every real tool
// call so the client — which re-evaluates the *real* mtime-cached merged user
// config (global + project) on every request — is the authoritative
// decision-maker. This is what lets a mid-session edit to .kilo/kilo.jsonc
// (e.g. a local `deny` overriding a global `allow`) take effect without a
// restart. The agent's own auto-allow/auto-deny is bypassed in favour of
// always asking. Cost: one IPC round-trip per tool call (no extra prompts).
// `*` is placed first so the DISABLE_PERMISSION `deny` keys (appended after)
// win under the agent's `findLast` rule resolution → meta-tools auto-deny
// with no round-trip.
const ASK_PERMISSION: Record<string, string> = {
  "*": "ask",
  bash: "ask",
  read: "ask",
  edit: "ask",
...
function computeKiloConfigContent(): Record<string, unknown> {
  ...
  // Replace the user's permission rules with the ask/deny override so the
  // agent routes every real tool call through requestPermission instead of
  // auto-allowing/denying from the user's files (see ASK_PERMISSION). The
  // client re-evaluates the real merged rules on every request.
  delete content.permission;
  content.permission = { ...ASK_PERMISSION, ...DISABLE_PERMISSION };
  return content;
}
...
function summarizeInputParts(input: unknown): string[] {
  if (!input || typeof input !== "object") return [];
  const obj = input as Record<string, unknown>;
  const parts: string[] = [];
  const seen = new Set<string>();
  for (const [key, label] of INPUT_FIELD_LABELS) {
    // Shell command aliases carry the same value; prefer `command` when both
    // are present.
    if (key === "cmd" && seen.has("command")) continue;
    // Shell calls may carry an explicit `workdir` and an agent-resolved `cwd`.
    if (key === "cwd" && seen.has("workdir")) continue;
    ...
    seen.add(key);
    if (key === "command" || key === "cmd") parts.push(`${C.green}$ ${val}${C.reset}`);
...
function newInputSummary(id: string, input: unknown): string {
  const parts = summarizeInputParts(input);
  const rendered = renderedInputSummaryParts.get(id) ?? new Set<string>();
  const newParts = parts.filter((part) => !rendered.has(part));
  for (const part of newParts) rendered.add(part);
  if (newParts.length > 0) renderedInputSummaryParts.set(id, rendered);
  return newParts.join(`  ${C.dim}|${C.reset}  `);
}

// Full multi-line render of every input part for a permission prompt's tool
// context (the force render path). Unlike newInputSummary (which dedups and
// joins on one line), this prints each part on its own line — command,
// workdir, and description stay scannable when the permission menu follows
// immediately under them. Parts are marked rendered so a later incremental
// newInputSummary doesn't reprint them.
function renderInputPartsMultiline(id: string, input: unknown): void {
  const parts = summarizeInputParts(input);
  if (parts.length === 0) return;
  const rendered = renderedInputSummaryParts.get(id) ?? new Set<string>();
  for (let i = 0; i < parts.length; i++) {
    rendered.add(parts[i]);
    const prefix = i === 0 ? "   " : `   ${C.dim}|${C.reset}  `;
    console.log(`${prefix}${parts[i]}`);
  }
  renderedInputSummaryParts.set(id, rendered);
}
...
function plainWhat(input: unknown): string {
  if (!input || typeof input !== "object") return "";
  const obj = input as Record<string, unknown>;
  for (const key of ["command", "cmd", "pattern", "query", "regex", "include", "glob", "filePath", "filepath", "path", "parentDir", "url", "description"]) {
    const val = obj[key];
    if (typeof val === "string" && val !== "") {
      if (key === "command" || key === "cmd") return val;
      return val;
    }
  }
  return "";
...
// Permission requests reuse the originating toolCallId but may replace its
// title with a path or description. Retain the latest originating call so the
// request can stay attached to the correct command block.
const observedToolCalls = new Map<string, ToolCallLike>();
let lastRenderedToolCallId: string | null = null;
...
// Input-summary parts already printed for each tool call. Inputs can arrive
// incrementally (for example, bash may announce its workdir before its command),
// so track individual parts rather than suppressing all later input updates.
const renderedInputSummaryParts = new Map<string, Set<string>>();
...
function mergeRawInput(base: unknown, update: unknown): unknown {
  if (!base || typeof base !== "object" || Array.isArray(base)) return update ?? base;
  if (!update || typeof update !== "object" || Array.isArray(update)) return update ?? base;
  return { ...(base as Record<string, unknown>), ...(update as Record<string, unknown>) };
}

function rememberToolCall(update: ToolCallLike): void {
  const id = update.toolCallId ?? update.title;
  if (!id) return;
  const previous = observedToolCalls.get(id);
  observedToolCalls.set(id, previous
    ? {
        ...previous,
        ...update,
        title: previous.title ?? update.title,
        kind: previous.kind ?? update.kind,
        rawInput: mergeRawInput(previous.rawInput, update.rawInput),
      }
    : update);
}

function permissionDisplayCall(update: ToolCallLike): ToolCallLike {
  const id = update.toolCallId ?? update.title;
  const original = id ? observedToolCalls.get(id) : undefined;
  if (!original) return update;
  return {
    ...update,
    title: original.title ?? update.title,
    kind: original.kind ?? update.kind,
    status: original.status ?? update.status,
    locations: update.locations ?? original.locations,
    rawInput: mergeRawInput(original.rawInput, update.rawInput),
  };
}

// ACP does not expose the permission key separately from its human-readable
// ToolCallUpdate. Derive the config key from structured input and tool kind so
// a path or a whole command is never persisted as the permission name.
function permissionToolName(update: ToolCallLike): string {
  const raw = update.rawInput && typeof update.rawInput === "object" && !Array.isArray(update.rawInput)
    ? update.rawInput as Record<string, unknown>
    : {};
  if (typeof raw.parentDir === "string" || Array.isArray(raw.directories)) return "external_directory";

  const id = update.toolCallId ?? update.title;
  const original = id ? observedToolCalls.get(id) : undefined;
  const known = new Set([
    "bash", "shell", "read", "edit", "write", "apply_patch", "grep", "glob",
    "webfetch", "websearch", "task", "skill", "recall", "todowrite",
  ]);
  for (const title of [original?.title, update.title]) {
    const normalized = title?.trim().toLowerCase();
    if (normalized && known.has(normalized)) return normalized;
  }

  const kind = (original?.kind ?? update.kind ?? "").toLowerCase();
  if (kind === "execute" || typeof raw.command === "string" || typeof raw.cmd === "string") return "bash";
  if (kind === "read") return "read";
  if (kind === "edit") {
    if (typeof raw.content === "string" || typeof raw.contents === "string") return "write";
    return "edit";
  }
  if (kind === "fetch") return "webfetch";
  if (kind === "search") {
    if (typeof raw.query === "string") return "websearch";
    return "grep";
  }

  const fallback = update.title?.trim();
  return fallback && !fallback.includes("/") && !/\s/.test(fallback) ? fallback : "this_tool";
}

// Cap a single over-long line, preserving both edges. Minified blobs and very
// long grep matches are common, so keep the start and end visible with a count
// of how many chars were dropped in the middle.
const MAX_LINE_CHARS = 1000;
function trimLine(line: string): string {
  if (line.length <= MAX_LINE_CHARS) return line;
  const edge = Math.floor((MAX_LINE_CHARS - 40) / 2);
  const omitted = line.length - edge * 2;
  return `${line.slice(0, edge)} ... (${omitted} chars truncated) ... ${line.slice(-edge)}`;
}

// Render grep/search tool output like regular grep: one `path:line: content`
// line per match, all gray. Like other tools, output is trimmed to MAX lines
// keeping both the head and the tail (with a "... (N lines truncated) ..."
// marker in the middle) so the start and end of the result set stay visible.
// Each rendered line is also char-trimmed so a huge match can't flood the
// terminal. If the tool reports a total match count, it's noted at the bottom.
function renderSearchOutput(text: string): void {
  ...
  let shown = lines;
  if (lines.length > MAX) {
    const headCount = Math.ceil(MAX / 2);
  ...
  for (const l of shown) console.log(`   ${C.dim}${trimLine(l)}${C.reset}`);
  if (foundTotal != null && foundTotal > lines.length) {
    console.log(`   ${C.dim}... (${foundTotal} total matches)${C.reset}`);
  }
}

// Keep tool output useful without letting a large file, command, or minified
// payload flood terminal scrollback. Preserve both ends because command errors
// and summaries commonly appear at the bottom.
function renderTrimmedOutput(text: string, maxLines: number): void {
...
function renderToolCall(
  ...
  // prompt): print the header + what it's doing. The forced (permission-context)
  // render lays input parts out one per line; the first announcement keeps the
  // compact single-line join.
  if (force || !announcedToolCalls.has(id)) {
    ...
    if (force) {
      renderInputPartsMultiline(id, update.rawInput);
    } else {
      const summary = newInputSummary(id, update.rawInput);
      if (summary) {
        console.log(`   ${summary}`);
      }
    }
    ...
    // Print input fields added by this update before considering whether its
    // status is a duplicate. Some agents add the bash command to a second
    // `in_progress` update after first announcing only its workdir.
    const summary = newInputSummary(id, update.rawInput);
    if (summary) {
      console.log(`   ${summary}`);
    }
  ...
  if (showBody && !isTodowrite && !knownTodowriteIds.has(id) && Array.isArray(update.content)) {
    ...
    for (let i = showFrom; i < total; i++) {
      ...
      if (c.type === "diff") {
        ...
        } else if (kind === "read" || title === "read") {
          renderTrimmedOutput(c.content.text, 20);
        } else if (kind === "compact" || title === "compact" || title === "compaction") {
          renderTrimmedOutput(c.content.text, 20);
        } else if (kind === "execute" || title === "bash" || title === "command" || title === "shell") {
          renderTrimmedOutput(c.content.text, 100);
        } else if (kind === "fetch") {
          renderTrimmedOutput(c.content.text, 200);
        } else {
          renderTrimmedOutput(c.content.text, 100);
        }
      }
...
// Render permission-only input (most importantly a shell command) as part of
// its originating tool block. If another parallel call printed in between,
// repeat the originating header so the request is still unambiguous.
function renderPermissionToolContext(update: ToolCallLike, repeat = false): void {
  const display = permissionDisplayCall(update);
  const id = display.toolCallId ?? display.title ?? "";
  if (repeat) {
    // A queued prompt may be far away from the original parallel announcement.
    // Re-render every input part, not only fields that arrived since then.
    renderedInputSummaryParts.delete(id);
    renderToolCall(display, false, true, true);
    return;
  }
  if (!announcedToolCalls.has(id) || lastRenderedToolCallId !== id) {
    renderToolCall(display, false, true, true);
    return;
  }

  const summary = newInputSummary(id, display.rawInput);
  if (summary) console.log(`   ${summary}`);
  const kind = (display.kind ?? "other").toLowerCase();
  if (kind === "edit" && !renderedEditDiffIds.has(id)) {
    if (renderEditDiffFromRaw(display.rawInput, id)) renderedEditDiffIds.add(id);
  }
  lastRenderedToolCallId = id;
}
...
const COMMAND_HELP: Record<string, string> = {
  ...
  "/sessions": "Show recent sessions across all providers, free models greyed out (arg = page; offset = arg×10, e.g. /sessions 2)",
  ...
  "/thinking": "Choose the thinking/reasoning (effort) level for the active model",
  "/toggle-thinking": "Toggle display of thinking blocks",
  "/models": "Switch the active model (alias: /model)",
  "/model": "Switch the active model (in place, from config + commented-out candidates)",
  ...
  "/codex-balance": "Show Codex (ChatGPT subscription) rate-limit usage",
  "/openrouter-balance": "Show OpenRouter credit balance and key usage (day/week/month)",
  "/kilo-gateway-balance": "Show your Kilo Gateway prepaid balance and Kilo Pass credits",
  "/kilo-gateway-logs": "Show recent non-free Kilo Gateway requests (arg = page; offset = arg×10, e.g. /kilo-gateway-logs 2)",
...
const IMPLEMENTED_COMMANDS = new Set(["/exit", "/quit", "/help", "/status", "/sessions", "/compact", "/model", "/models", "/thinking", "/codex-balance", "/openrouter-balance", "/kilo-gateway-balance", "/kilo-gateway-logs"]);
const STUB_COMMANDS = new Set(
  COMMANDS.filter((c) => !IMPLEMENTED_COMMANDS.has(c)),
);
// /kilo-gateway-logs and /sessions take an optional integer page-index arg,
// so they are the commands excluded from the no-arg rejection set.
const PAGED_COMMANDS = new Set(["/kilo-gateway-logs", "/sessions"]);
const NO_ARG_COMMANDS = new Set([...COMMANDS.filter((c) => !PAGED_COMMANDS.has(c)), "/h", "/?"]);
...
function buildAgentEnv(): NodeJS.ProcessEnv {
  ...
  // the user's real global + local configs, plus the ask/deny permission
  // override that forces the agent to route every real tool call through
  // requestPermission (see ASK_PERMISSION above).
  ...
  env.KILO_CONFIG_CONTENT = JSON.stringify({
    ...config,
    ...kiloConfigContent,
    permission: { ...ASK_PERMISSION, ...DISABLE_PERMISSION },
  });
...
function completePath(token: string): [string[], string] {
  ...
  for (const e of entries) {
    ...
    // Directories are listed WITHOUT a trailing "/": tab cycling selects the
    // bare name, and the user descends by typing "/" and pressing Tab again
    // (which starts a fresh completion inside that directory).
    hits.push(prefix + e);
  }
  hits.sort();
  return [hits, token];
...
// Matches Ctrl+C as reported by terminals using the kitty keyboard protocol
// (CSI u form): codepoint 3 (ETX) or 99 ('c') with a ctrl-bearing modifier
// (5=ctrl, 6=shift+ctrl, 7=alt+ctrl, 8=shift+alt+ctrl), optionally carrying a
// ":event" suffix. Raw 0x03 is detected separately via Buffer.indexOf.
const KITTY_CTRL_C = /\x1b\[(?:3|99);[5-8](?::\d)?u/;
...
const setInputTarget = (target: InputTarget): void => {
  ...
  const handler = (chunk: Buffer) => {
    ...
    // Some terminals report Ctrl+C via the kitty keyboard protocol (CSI u)
    // instead of a raw 0x03 byte; detect that form here too so the search
    // (and the rest of the Ctrl+C handling below) treats it identically.
    const hasCtrlC = idx03 !== -1 || KITTY_CTRL_C.test(chunk.toString("latin1"));
    ...
    if (hasCtrlC) {
      ...
      // /model's search view owns rawInput: Ctrl+C backs out of the search to
      // the favourites menu (instead of arming the double-press exit or
      // aborting a turn's cancel controller — no turn is running here). Ctrl+D
      // below still cancels /model entirely.
      if (target === "main" && inModelSearch) {
        modelSearchToMenu = true;
        rawInput.abort();
        return;
      }
      ...
      if (idx !== -1) {
        ...
        if (rest.length > 0) {
          ...
          if (brkIdx !== -1) {
            ...
            // Drop paste content in model search mode; paste is not allowed there.
            if (target === "main" && !inModelSearch) {
            ...
            if (trailing.length > 0 && target === "main" && !inModelSearch) {
              rawInput.handleBytes(trailing);
            }
          } else {
            if (!inModelSearch) pasteResidue = rest;
          }
        }
      }
      return;
    ...
    if (str.includes("\x1b[201~")) {
      ...
      if (brkIdx !== -1) {
        ...
        // Drop paste content in model search mode; paste is not allowed there.
        if (pasteBuf.length > 0 && target === "main" && !inModelSearch) {
        ...
        if (pasteResidue.length > 0 && target === "main" && !inModelSearch) {
    ...
    if (inBracketedPaste && target === "main") {
      ...
      if (inModelSearch) {
        ...
        if (brkIdx !== -1) {
          ...
          pasteResidue = Buffer.alloc(0);
          rawInput.setPasteMode(false);
          // trailing non-paste bytes after end marker are also dropped in search mode
        } else {
          pasteResidue = Buffer.alloc(0);
        }
        return;
...
// True while /model's search view owns the shared rawInput (so its completer
// drives the Tab-cycling overlay). The stdin handler checks this so Ctrl+C /
// Ctrl+D during the search cancel the search (returning to the menu / prompt)
// instead of arming the double-Ctrl+C-to-exit or killing the program.
let inModelSearch = false;
// Set when the user cancels the model search via Ctrl+C / Ctrl+D, so the
// search read() (which resolves "" on abort) can distinguish a real cancel
// from an empty submit and bail out of /model entirely.
let modelSearchAbort = false;
// Set when the user presses Ctrl+C during the model search: unlike a full
// abort (Ctrl+D), Ctrl+C only backs out of the search to the favourites menu
// instead of leaving /model altogether.
let modelSearchToMenu = false;
...
// Model the *active* session actually uses, as reported by the agent when the
// session was loaded/resumed (via the `session/load` response's
// `configOptions`). Null for fresh sessions, where the agent falls back to the
// configured `model` — in that case printStatus shows the config value. This
// mirrors regular kilo, which keeps the model a session was started with even
// when the live config later changes.
let activeSessionModel: string | null = null;

// Full list of models the active session can be switched to, as reported by
// the agent in the `session/load` / `session/new` response's `configOptions`.
// This is the authoritative "available models" set: it includes built-in
// providers (e.g. the `kilo` free dispatcher), configured providers, AND
// providers the user is authenticated for even when they aren't listed in the
// config file's `provider` block. Null when the agent doesn't expose model
// config options. Refreshed on /model switch from the set_config_option
// response. Used by /model's "search all available models" mode.
let sessionModelOptions: { value: string; name: string }[] | null = null;

// The thinking/reasoning (effort) level the active session actually uses, from
// the agent's `effort` config option in `configOptions`. Null when the active
// model has no variants (no thinking control) or the agent didn't expose the
// effort option. Refreshed alongside activeSessionModel from the
// session/load, session/new, and session/set_config_option responses.
let activeEffort: string | null = null;

// Available effort/thinking levels for the active model, from the agent's
// `effort` select option. Null when the model has no effort levels. Drives the
// `thinking:` line in /status and the /thinking picker.
let sessionEffortOptions: { value: string; name: string }[] | null = null;
...
// Extract the model select from a `session/load` / `session/new` /
// `session/set_config_option` response's `configOptions` array. The agent
// returns a select option with `id: "model"`, `type: "select"`, whose
// `currentValue` is the model id the session actually uses (e.g.
// `provider/model`) and whose `options` is the full list of models the agent
// knows — i.e. everything the session can be switched to with
// `session/set_config_option`. That list reflects providers the agent loaded
// (built-ins like the `kilo` free dispatcher, configured providers, AND
// providers the user is authenticated for even if not in the config file), so
// it's the authoritative "available models" set — more accurate than parsing
// the config's `provider` block. Returns null when the agent doesn't expose
// model config options.
function extractModelSelect(
  configOptions: unknown,
): { current: string | null; options: { value: string; name: string }[] } | null {
  if (!Array.isArray(configOptions)) return null;
  for (const opt of configOptions) {
    if (!opt || typeof opt !== "object") continue;
    const o = opt as Record<string, unknown>;
    if (o.id !== "model" || o.type !== "select") continue;
    const current = typeof o.currentValue === "string" && o.currentValue ? o.currentValue : null;
    ...
    if (Array.isArray(optsRaw)) {
      for (const item of optsRaw) {
        ...
        if (typeof it.value === "string" && "name" in it) {
          options.push({ value: it.value, name: String(it.name ?? it.value) });
          continue;
        }
        // Grouped: { group, name, options: [...] }
        if (Array.isArray(it.options)) {
          for (const s of it.options) {
            if (!s || typeof s !== "object") continue;
            const sv = s as Record<string, unknown>;
            if (typeof sv.value === "string") {
              options.push({ value: sv.value, name: String(sv.name ?? sv.value) });
            }
          }
        }
      }
...
// Extract the thinking/effort select from a `session/load` / `session/new` /
// `session/set_config_option` response's `configOptions` array. The agent
// returns a select option whose `id` is "effort" (the kilo convention; see
// ../kilocode-src packages/opencode/src/acp/config-option.ts buildEffortSelectOption),
// whose `currentValue` is the variant the session actually uses (already
// resolved by kilo from saved/default/first variant), and whose `options` is
// the set of selectable thinking levels for the active model. Mirrors
// extractModelSelect above. Accepts "thinking"/"reasoning" as a defensive
// fallback in case a future build renames the id. Returns null when the agent
// doesn't expose an effort option (the model has no thinking control).
function extractEffortSelect(
  configOptions: unknown,
): { current: string | null; options: { value: string; name: string }[] } | null {
  if (!Array.isArray(configOptions)) return null;
  for (const opt of configOptions) {
    ...
    if (id !== "effort" && id !== "thinking" && id !== "reasoning") continue;
    const current = typeof o.currentValue === "string" && o.currentValue ? o.currentValue : null;
    const options: { value: string; name: string }[] = [];
    const optsRaw = o.options;
    if (Array.isArray(optsRaw)) {
...
// ACP creates a fresh session with the model's built-in default/first variant,
// but unlike kilo's interactive UI it does not restore model.json's saved
// per-model variant. Apply that saved value before the fresh session is shown
// or prompted, provided the agent says it is valid for the selected model.
async function restoreFreshSessionEffort(
  ctx: acp.ClientContext,
  sessionId: string,
  model: { current: string | null; options: { value: string; name: string }[] } | null,
  effort: { current: string | null; options: { value: string; name: string }[] } | null,
): Promise<{
  model: typeof model;
  effort: typeof effort;
}> {
  const modelId = model?.current;
  if (!modelId || !effort || effort.options.length === 0) return { model, effort };

  const saved = readModelState().variant?.[modelId];
  if (!saved || saved === effort.current || !effort.options.some((option) => option.value === saved)) {
    return { model, effort };
  }
  ...
  try {
    ...
    return {
      model: extractModelSelect(configOptions) ?? model,
      effort: extractEffortSelect(configOptions) ?? { current: saved, options: effort.options },
    };
  } catch {
    // Keep the effort selected by the agent if restoring saved state fails.
    return { model, effort };
  }
}
...
async function resolveInitialSession(
  ...
  summary: string;
  model: string | null;
  modelOptions: { value: string; name: string }[] | null;
  effort: string | null;
  effortOptions: { value: string; name: string }[] | null;
}> {
  const cwd = process.cwd();
  ...
  if (target) {
    try {
      const loadRes = await ctx.request(acp.methods.agent.session.load, {
        sessionId: target,
        cwd,
        mcpServers: [],
      });
      // The load response carries the session's current config options,
      // including the model the resumed session actually uses — which may
      // differ from the live config `model` (regular kilo pins a session to
      // the model it was started with). The model select's `options` is the
      // full set of models the session can be switched to. Surface both so
      // printStatus shows the real model and /model can offer every available
      // model (incl. auth-only providers and the kilo free dispatcher).
      const sel = extractModelSelect(
        (loadRes as { configOptions?: unknown } | null)?.configOptions,
      );
      const esel = extractEffortSelect(
        (loadRes as { configOptions?: unknown } | null)?.configOptions,
      );
      ...
      if (name) sessionNames.set(target, name);
      writeLastSession(target);
      return {
        session: ms,
        name,
        summary,
        model: sel?.current ?? null,
        modelOptions: sel?.options ?? null,
        effort: esel?.current ?? null,
        effortOptions: esel?.options ?? null,
      };
    } catch (err) {
      console.error(
  ...
  // A fresh session uses the config `model` by default; capture the model
  // select (current value + available options) from the session/new response
  // too, so /model works identically for fresh and resumed sessions.
  let sel = extractModelSelect(
    (fresh.newSessionResponse as { configOptions?: unknown } | null)?.configOptions,
  );
  let esel = extractEffortSelect(
    (fresh.newSessionResponse as { configOptions?: unknown } | null)?.configOptions,
  );
  ({ model: sel, effort: esel } = await restoreFreshSessionEffort(
    ctx,
    fresh.sessionId,
    sel,
    esel,
  ...
  return {
    session: fresh as unknown as MainSession,
    name: null,
    summary: "",
    model: sel?.current ?? null,
    modelOptions: sel?.options ?? null,
...
async function handleCompact(
  ...
  const freshSession = await ctx.buildSession(process.cwd()).start();
  // The fresh session uses the config `model` by default; refresh the pinned
  // model + available-models list from its session/new response so /model and
  // /status stay accurate after compaction.
  let freshSel = extractModelSelect(
    (freshSession.newSessionResponse as { configOptions?: unknown } | null)?.configOptions,
  );
  let freshEffortSel = extractEffortSelect(
    (freshSession.newSessionResponse as { configOptions?: unknown } | null)?.configOptions,
  );
  ({ model: freshSel, effort: freshEffortSel } = await restoreFreshSessionEffort(
    ctx,
    freshSession.sessionId,
    freshSel,
    freshEffortSel,
  ));
  activeSessionModel = freshSel?.current ?? null;
  sessionModelOptions = freshSel?.options ?? null;
  // The compacted-into fresh session may have a different model/variant set;
  // refresh the effort state from the same response so /status and /thinking
  // stay accurate after compaction.
  activeEffort = freshEffortSel?.current ?? null;
  sessionEffortOptions = freshEffortSel?.options ?? null;
  if (activeSessionModel) recordRecentModel(activeSessionModel);
  const fresh = freshSession as unknown as MainSession;
  writeLastSession(fresh.sessionId);
  recordUsage(fresh.sessionId, "main", {});
  setSession(fresh);
...
// /model: switch the active session's model in place. Re-reads the kilo config
// fresh on every call (so newly added/commented models appear immediately).
//
// Two views:
//  - Favourites (top): every `"model": "<value>"` found in the config file —
//    both the active one and commented-out lines — deduped in order of first
//    appearance. `kilo/kilo-auto/free` (the free dispatcher) is always kept as
//    the first favourite. These are the models you've explicitly mentioned in
//    the config, treated as favourites.
//  - Search (bottom, 's'): a substring search over every model the agent
//    actually has available — i.e. the `options` from the session's model
//    config option, which reflects built-in providers (kilo free), configured
//    providers, AND providers you're authenticated for even when they're not
//    in the config's `provider` block. A blank search lists the `kilo/*`
//    models.
//
// Availability is taken from the agent (the authoritative source), not from
// the config's `provider` block: a favourite whose value isn't in the agent's
// available set is flagged `[unavailable]` (e.g. a commented-out `openrouter/*`
// when you aren't authenticated for openrouter). Choosing a model sends
// `session/set_config_option` (configId "model") to the current session, so
// the context is kept and only the model used for subsequent turns changes.
// Temporary: nothing is written to the config file.
async function handleModel(ctx: acp.ClientContext): Promise<void> {
  if (!activeSession) {
    console.log("\n⚠️  No active session to switch the model for.");
    return;
  }

  // --- Favourites: from the config file (re-read fresh) ---
  const cfgPath = findKiloConfigPath();
  let raw = "";
  if (cfgPath) {
    try {
      raw = fs.readFileSync(cfgPath, "utf-8");
    } catch (err) {
      console.log(`\n⚠️  Could not read config ${displayConfigPath(cfgPath)}: ${(err as Error).message}`);
      return;
    }
  }
  let activeMainModel: string | null = null;
  try {
    const parsed = parseJsonc(raw) as Record<string, unknown> | null;
    if (parsed && typeof parsed.model === "string") activeMainModel = parsed.model;
  } catch {
    /* ignore parse error; the regex below still yields candidates */
  }
  const favValues: string[] = [];
  const seen = new Set<string>();
  ...
  if (activeMainModel && !seen.has(activeMainModel)) favValues.unshift(activeMainModel);
  // Always offer the kilo free dispatcher first — it's built-in and needs no
  // user-supplied API key.
  const KILO_FREE = "kilo/kilo-auto/free";
  if (!seen.has(KILO_FREE)) favValues.unshift(KILO_FREE);

  // --- Available models: from the agent (authoritative) ---
  const opts = sessionModelOptions ?? [];
  const availMap = new Map<string, string>();
  const availSet = new Set<string>();
  for (const o of opts) {
    availSet.add(o.value);
    availMap.set(o.value, o.name);
  }
  const haveAgentList = sessionModelOptions !== null;
  const currentModel = activeSessionModel ?? activeMainModel;
  // If the session's current model isn't among the favourites, surface it as an
  // extra trailing entry (marked with a star) so the user can re-select/see it
  // without leaving /model. favValues drives both the printed list and the
  // numeric selection, so appending here makes it pickable by its number too.
  if (currentModel && !favValues.includes(currentModel)) {
    favValues.push(currentModel);
  }

  // --- Recently-used models (per-project) surfaced as extra greyed, numbered
  // entries — up to RECENT_MODELS_SHOW models that aren't already favourites
  // (or the current-model star entry above) and are still available on this
  // session. Only shown when the agent published an available-models list we
  // can verify presence against ("only if those models present"). These keep
  // real selectable numbers continuing the favourites list.
  const recentStartIdx = favValues.length;
  if (haveAgentList) {
    for (const r of readRecentModels()) {
      if (favValues.length - recentStartIdx >= RECENT_MODELS_SHOW) break;
      if (favValues.includes(r)) continue;
      if (!availSet.has(r)) continue;
      favValues.push(r);
    }
  }

  // Apply a model to the current session in place. Returns true on success and
  // refreshes the pinned model + available list from the response. The caller
  // records and announces the switch only after the thinking picker finishes,
  // because Ctrl+C there restores the previous model.
  const applySwitch = async (value: string): Promise<boolean> => {
    try {
      const res = await ctx.request(acp.methods.agent.session.setConfigOption, {
      ...
      const sel = extractModelSelect(
        (res as { configOptions?: unknown } | null)?.configOptions,
      );
      const esel = extractEffortSelect(
        (res as { configOptions?: unknown } | null)?.configOptions,
      );
      activeSessionModel = sel?.current ?? value;
      if (sel?.options) sessionModelOptions = sel.options;
      // The new model may have different (or no) thinking levels; refresh them
      // from the same response so /status and /thinking reflect the switched
      // model's effort options rather than the previous model's.
      activeEffort = esel?.current ?? null;
      sessionEffortOptions = esel?.options ?? null;
      return true;
    } catch (err) {
      const msg = (err as Error)?.message ?? String(err);
      console.log(`${C.red}⚠️  Could not switch model:${C.reset} ${msg}`);
      console.log(
        `${C.dim}   The agent rejected this model. If its provider isn't available (authenticated/configured), enable it in ${cfgPath ? displayConfigPath(cfgPath) : "your config"} and restart kilo-readline.${C.reset}`,
      );
      return false;
    }
  };
  ...
  const finishSwitch = (): void => {
    ...
    recordRecentModel(activeSessionModel);
    console.log(`${C.green}✓${C.reset} Model switched to ${C.bold}${activeSessionModel}${C.reset} (session ${activeSession!.sessionId}).`);
  };

  const switchWithThinking = async (value: string): Promise<void> => {
    const previousModel = currentModel;
    const previousEffort = activeEffort;
    if (!(await applySwitch(value))) return;

    const accepted = await promptThinkingAfterModelSwitch(ctx);
    if (accepted) {
      finishSwitch();
      return;
    }

    // Effort options are model-specific and only arrive in the switch response,
    // so Ctrl+C is implemented as a rollback of the provisional model switch.
    if (previousModel && previousModel !== activeSessionModel) {
      const restored = await applySwitch(previousModel);
      if (
        restored && previousEffort != null && activeEffort !== previousEffort &&
        sessionEffortOptions?.some((o) => o.value === previousEffort)
      ) {
        await applyEffortLevel(ctx, previousEffort);
      }
    }
    console.log(`${C.dim}Model selection cancelled.${C.reset}`);
  };

  const printFavourites = (): void => {
    const src = cfgPath ? `  ${C.dim}(favourites from ${displayConfigPath(cfgPath!)})${C.reset}` : "";
    console.log(`\n${C.bold}Switch model${C.reset}${src}`);
    for (let i = 0; i < favValues.length; i++) {
      const v = favValues[i];
      const isCurrent = v === currentModel;
      const available = haveAgentList ? availSet.has(v) : null;
      const name = availMap.get(v);
      const isRecent = i >= recentStartIdx;
      // A leading star marks the model the session currently uses (and the
      // trailing "(current)" suffix is dropped to avoid redundancy with it).
      const star = isCurrent ? `${C.yellow}★${C.reset} ` : "";
      if (isRecent) {
        // Greyed, numbered trailing entry: a model recently used in this
        // project that isn't an explicit favourite. Keep its real number so
        // it's selectable like any favourite, but dim the whole line.
        const label = ` ${C.dim}(recently used)${C.reset}`;
        console.log(`${C.dim}   ${i + 1}. ${C.reset}${star}${C.dim}${v}${C.reset}${label}`);
        continue;
      }
      const unavail = available === false ? ` ${C.red}[unavailable]${C.reset}` : "";
      const nameSuffix = name && name !== v ? `  ${C.dim}${name}${C.reset}` : "";
      console.log(`   ${i + 1}. ${star}${v}${unavail}${nameSuffix}`);
    }
    const searchHint = haveAgentList
      ? `   ${C.dim}s. Search all available models (${opts.length} models, incl. kilo free)${C.reset}`
      : `   ${C.dim}s. Search available models${C.reset}`;
    console.log(searchHint);
  };

  startAwaitingUserInput();
  try {
    let view: "menu" | "search" = "menu";
    while (true) {
      if (view === "menu") {
        printFavourites();
        process.stdout.write("\nChoose (number, 's' to search, 'c' to cancel): ");
        const ans = await readSingleLine({ allow: "0123456789sScC" });
        if (cancelController.signal.aborted) {
          process.stdout.write("\n");
          return;
        }
        ...
        if (t === "s") {
          if (!haveAgentList) {
            console.log(`${C.dim}(The agent didn't publish a model list — nothing to search.)${C.reset}`);
            continue;
          }
          view = "search";
          continue;
        ...
        if (t === "") continue;
        const ch = parseInt(t, 10) - 1;
        if (!(ch >= 0 && ch < favValues.length)) {
          console.log("Invalid choice.");
          continue;
        }
        const v = favValues[ch];
        if (haveAgentList && !availSet.has(v)) {
          console.log(
            `${C.red}${v} is not available on this session${C.reset} (its provider isn't loaded/authenticated). Try 's' to search available models, or enable the provider and restart.`,
          );
          continue;
        }
        process.stdout.write("\n");
        await switchWithThinking(v);
        process.stdout.write("\n");
        return;
      }

      // view === "search"
      // Interactive search reusing the shared rawInput + its Tab-cycling
      // completion overlay. Tab has two modes, decided by the token under the
      // cursor (the last whitespace-delimited chunk):
      //  - full-path mode (token contains "/"): Tab cycles full model ids by
      //    prefix — typing a provider-qualified path autocompletes like a path.
      //  - word mode (token has no "/"): Tab cycles the individual words that
      //    occur across model ids/names (e.g. "son" -> "sonnet"), so the user
      //    builds a multi-word query like "claude sonnet" word by word.
      // Enter resolves the whole query: an exact full model id (surrounded by
      // only whitespace) is selected immediately; otherwise the query is
      // AND-matched per word against every model's value/name and the result
      // must narrow to one model (or the user presses Tab to cycle). 'm'
      // jumps to favourites; Ctrl+C backs out to the favourites menu; Ctrl+D /
      // 'c' cancels /model entirely.
      const modelOpts: ModelOption[] = opts;
      const wordList = extractModelWords(modelOpts);
      const modelCompleter = (line: string, cursorCol: number): [string[], string] => {
        // Extract the token under the cursor the same way RawInput does
        // (whitespace-delimited), so completer and cycling agree.
        const before = line.slice(0, cursorCol);
        const m = before.match(/(\S+)$/);
        const token = m ? m[1] : "";
        if (token === "") return [[], line];
        if (isFullPathToken(token)) {
          return [matchModelPaths(token, modelOpts), line];
        }
        return [matchModelWords(token, wordList), line];
      };

      let picked: string | null = null;
      modelSearchAbort = false;
      modelSearchToMenu = false;
      setInputTarget("main");
      rawInput.setPrompt("search models (Tab=autocomplete, Ctrl+C=back)> ");
      rawInput.setCompleter(modelCompleter);
      // Disable prompt-history navigation here: the search box is a free-text
      // query, so Up/Down and Ctrl+R must not load old prompt-history entries
      // and clobber what the user is typing.
      rawInput.setHistoryEnabled(false);
      // Reserve 3 rows at the bottom: 2 overlay rows + 1 prompt row.
      rawInput.setBottomPinned(true, 3);
      inModelSearch = true;
      let text = "";
      try {
        text = await rawInput.read(false);
      } finally {
        inModelSearch = false;
        rawInput.setBottomPinned(false);
        rawInput.setCompleter(completer);
        rawInput.setPrompt("kilo> ");
        rawInput.setHistoryEnabled(true);
        setInputTarget("confirm");
      }
      if (modelSearchAbort) {
        process.stdout.write("\n");
      ...
      // Ctrl+C only backs out of the search to the favourites menu (it does
      // not leave /model). rawInput.abort() already echoed "^C" and resolved
      // the read with "", so just drop back to the menu view.
      if (modelSearchToMenu) {
        view = "menu";
        continue;
      }
      ...
      if (t === "") continue; // blank: stay in search (Tab lists kilo models)
      // The primary picker is the Tab-cycling overlay: type a query, press Tab
      // to fill the input with a match and cycle the 2-row variant list, then
      // Enter confirms. So on Enter we only switch when the input is (or maps
      // to) exactly one model — otherwise re-prompt so the user uses Tab.
      const exact = exactModelMatch(text, opts);
      if (exact) {
        picked = exact.value;
      } else {
        const matches = matchModelsPure(text, opts);
        if (matches.length === 1) {
          picked = matches[0].value;
        } else if (matches.length === 0) {
          console.log(`${C.dim}No models match "${text.trim()}" — refine.${C.reset}`);
        } else {
          console.log(
            `${C.dim}${matches.length} models match "${text.trim()}" — press Tab to cycle, or refine the query.${C.reset}`,
          );
          // Only the first two rows of matching model ids are printed (just
          // enough to hint what matched); the rest are reachable via the
          // Tab-cycling overlay, so we don't flood the screen with a full grid.
          const cols = process.stdout.columns || 80;
          const ids = matches.map((o) => o.value);
          const maxW = ids.reduce((m, v) => Math.max(m, visibleWidth(v)), 1);
          const colW = Math.min(maxW + 2, cols);
          const perRow = Math.max(1, Math.floor(cols / colW));
          const CAP = perRow * 2;
          const shown = ids.slice(0, CAP);
          let line = "";
          for (let i = 0; i < shown.length; i++) {
            line += padVisible(shown[i], colW - 2) + "  ";
            if ((i + 1) % perRow === 0) {
              console.log(`${C.dim}${line.trimEnd()}${C.reset}`);
              line = "";
            }
          }
          if (line) console.log(`${C.dim}${line.trimEnd()}${C.reset}`);
          if (ids.length > CAP) {
            console.log(
              `${C.dim}... and ${ids.length - CAP} more — press Tab to cycle, or refine to narrow.${C.reset}`,
            );
          }
        }
      }
      if (picked && haveAgentList && !availSet.has(picked)) {
        console.log(`${C.red}${picked} is not available on this session.${C.reset}`);
        picked = null;
      }
      if (picked) {
        process.stdout.write("\n");
        await switchWithThinking(picked);
        process.stdout.write("\n");
        return;
      }
      continue;
...
// Apply an effort/thinking level to the active session in place: live via
// session/set_config_option (configId "effort") AND persisted as the model's
// default variant to kilo's state file (model.json `variant` map) so it
// survives restarts. Refreshes the pinned effort + model state from the
// response. Returns { ok, effort } (effort is the resolved level on success).
async function applyEffortLevel(
  ctx: acp.ClientContext,
  value: string,
): Promise<{ ok: boolean; effort: string | null }> {
  if (!activeSession) return { ok: false, effort: null };
  try {
    const res = await ctx.request(acp.methods.agent.session.setConfigOption, {
      sessionId: activeSession.sessionId,
      configId: "effort",
    ...
    const configOptions = (res as { configOptions?: unknown } | null)?.configOptions;
    const sel = extractEffortSelect(configOptions);
    activeEffort = sel?.current ?? value;
    if (sel?.options) sessionEffortOptions = sel.options;
    // The agent republishes the model select too; refresh it so a variant
    // encoded in the model id (provider/model/variant) stays consistent.
    const ms = extractModelSelect(configOptions);
    if (ms?.current) activeSessionModel = ms.current;
    if (ms?.options) sessionModelOptions = ms.options;
    if (activeSessionModel) saveModelVariant(activeSessionModel, activeEffort ?? value);
    return { ok: true, effort: activeEffort };
  } catch (err) {
    const msg = (err as Error)?.message ?? String(err);
    console.log(`${C.red}⚠️  Could not set thinking level:${C.reset} ${msg}`);
    return { ok: false, effort: null };
  }
}

// After a model switch, if the new model exposes thinking/effort levels, prompt
// the user to pick one. The pre-selected default is the model's saved variant
// from model.json when valid, then the level the agent already chose, then the
// first available level. Empty input accepts the default; 'c' skips without
// changing what the agent already set. Returns false only for Ctrl+C, allowing
// /model to roll back its provisional model switch.
async function promptThinkingAfterModelSwitch(ctx: acp.ClientContext): Promise<boolean> {
  const opts = sessionEffortOptions ?? [];
  if (opts.length === 0) return true; // model has no thinking/effort levels
  const savedEffort = activeSessionModel
    ? readModelState().variant?.[activeSessionModel]
    : undefined;
  let defaultIdx = savedEffort != null ? opts.findIndex((o) => o.value === savedEffort) : -1;
  if (defaultIdx < 0) {
    defaultIdx = activeEffort != null ? opts.findIndex((o) => o.value === activeEffort) : -1;
  }
  if (defaultIdx < 0) defaultIdx = 0;
  const defaultValue = opts[defaultIdx].value;
  const defDisplay = formatEffortDisplay(defaultValue, opts);

  console.log(`\n${C.bold}Thinking level${C.reset}  ${C.dim}(for ${activeSessionModel ?? "this model"})${C.reset}`);
  for (let i = 0; i < opts.length; i++) {
    const o = opts[i];
    const isDefault = i === defaultIdx;
    const star = isDefault ? `${C.yellow}★${C.reset} ` : "";
    const display = formatEffortDisplay(o.value, opts);
    const nameSuffix = display !== o.value ? `  ${C.dim}${display}${C.reset}` : "";
    console.log(`   ${i + 1}. ${star}${o.value}${nameSuffix}`);
  }

  while (true) {
    process.stdout.write(`\nChoose (number, Enter=${defDisplay}, 'c' to skip): `);
    const ans = await readSingleLine({ allow: "0123456789cC" });
    if (cancelController.signal.aborted) {
      process.stdout.write("\n");
      return false;
    }
    ...
    if (t === "") {
      // Accept the computed default. If the agent already set it, just persist.
      if (activeEffort === defaultValue) {
        if (activeSessionModel) saveModelVariant(activeSessionModel, defaultValue);
        console.log(`${C.dim}Keeping ${defDisplay}${C.reset}`);
      } else {
        const r = await applyEffortLevel(ctx, defaultValue);
        if (r.ok) console.log(`${C.green}✓${C.reset} Thinking level set to ${C.bold}${defDisplay}${C.reset}.`);
      }
      return true;
    }
    const ch = parseInt(t, 10) - 1;
    if (!(ch >= 0 && ch < opts.length)) {
      console.log("Invalid choice.");
      continue;
    }
    const v = opts[ch].value;
    if (activeEffort === v) {
      if (activeSessionModel) saveModelVariant(activeSessionModel, v);
      console.log(`${C.dim}Already at ${formatEffortDisplay(v, opts)}${C.reset}`);
      return true;
    }
    const r = await applyEffortLevel(ctx, v);
    if (r.ok) console.log(`${C.green}✓${C.reset} Thinking level set to ${C.bold}${formatEffortDisplay(v, opts)}${C.reset}.`);
    return true;
  }
}


// session's model. The model's effort levels come from the agent's `effort`
// config option (captured at session load/switch into sessionEffortOptions).
// The choice is applied live via session/set_config_option (configId "effort")
// AND persisted to kilo's state file (model.json `variant` map) so it survives
// restarts — mirroring kilo's own saveVariant, which ACP's setVariant does not
// call. A model with no effort levels prints an info line and returns.
async function handleThinking(ctx: acp.ClientContext): Promise<void> {
  if (!activeSession) {
    console.log("\n⚠️  No active session.");
    return;
  }
  const opts = sessionEffortOptions ?? [];
  if (opts.length === 0) {
    const m = activeSessionModel ?? "(this model)";
    console.log(`\nℹ️  ${m} has no thinking/effort levels to choose.`);
    return;
  }

  const currentModel = activeSessionModel ?? "(unknown model)";

  // Apply the chosen effort level (delegates to the shared applyEffortLevel so
  // the live set_config_option + state refresh + model.json persist are reused
  // by /thinking and by the post-switch prompt). Prints the success line.
  const applyEffort = async (value: string): Promise<boolean> => {
    const r = await applyEffortLevel(ctx, value);
    if (r.ok) {
      const display = formatEffortDisplay(activeEffort, sessionEffortOptions ?? []);
      console.log(`${C.green}✓${C.reset} Thinking level set to ${C.bold}${display}${C.reset} and saved as default for ${currentModel}.`);
    }
    return r.ok;
  };

  const printLevels = (): void => {
    console.log(`\n${C.bold}Switch thinking level${C.reset}  ${C.dim}(effort levels for ${currentModel})${C.reset}`);
    for (let i = 0; i < opts.length; i++) {
      const o = opts[i];
      const isCurrent = activeEffort != null && o.value === activeEffort;
      const star = isCurrent ? `${C.yellow}★${C.reset} ` : "";
      const display = formatEffortDisplay(o.value, opts);
      const nameSuffix = display !== o.value ? `  ${C.dim}${display}${C.reset}` : "";
      console.log(`   ${i + 1}. ${star}${o.value}${nameSuffix}`);
...
async function main(): Promise<void> {
  ...
  const handleRequestPermission = async (
    ...
    if (isUnknownOrInvalidToolCall(params.toolCall)) {
      ...
      const toolNameHead = permissionToolName(tcHead);
      const tcId = tcHead.toolCallId ?? tcHead.title ?? "";
      ...
      permissionSeenForToolCall.add(tcId);
      ...
      if (localMatch) {
        ...
        if (allowOnce) {
          renderPermissionToolContext(tcHead);
      ...
      startAwaitingUserInput();
      try {
        renderPermissionToolContext(tcHead, true);
        const permissionScope = ruleToolName(toolNameHead);
        console.log(`   ${C.yellow}🔐 Permission requested: ${permissionScope}${C.reset}`);
  ...
  // Only one permission prompt may own stdin at a time. Parallel tool calls can
  // deliver concurrent client requests even when an agent usually queues them.
  let permissionRequestQueue: Promise<void> = Promise.resolve();
  const enqueuePermissionRequest = (
    params: acp.RequestPermissionRequest,
  ): Promise<acp.RequestPermissionResponse> => {
    const response = permissionRequestQueue.then(() => handleRequestPermission(params));
    permissionRequestQueue = response.then(() => undefined, () => undefined);
    return response;
  };
  ...
  try {
    await acp
      ...
      .onRequest(acp.methods.client.session.requestPermission, (ctx) =>
        enqueuePermissionRequest(ctx.params),
      )
      ...
      .connectWith(stream, async (ctx) => {
        ...
        const { session: initialSession, name: resumedName, summary: resumedSummary, model: resumedModel, modelOptions: resumedModelOptions, effort: resumedEffort, effortOptions: resumedEffortOptions } = await resolveInitialSession(ctx);
        let session: MainSession = initialSession;
        activeSession = session;
        // Pin the displayed model to the one the resumed/fresh session actually
        // uses (null only when the agent didn't expose model config options →
        // printStatus falls back to the config `model`). Also capture the full
        // list of models the session can switch to, so /model can offer every
        // available model (incl. auth-only providers and kilo free).
        activeSessionModel = resumedModel;
        sessionModelOptions = resumedModelOptions;
        // Pin the displayed thinking/effort level to the one the session
        // actually uses, plus the available levels for /thinking. Null when the
        // active model has no thinking control (no variants) or the agent
        // didn't expose the effort option — in that case /status omits the
        // thinking line and /thinking reports the model has no levels.
        activeEffort = resumedEffort;
        sessionEffortOptions = resumedEffortOptions;
        if (resumedModel) recordRecentModel(resumedModel);
        ...
        let codexFiveHourRemaining: number | null = null;
        const refreshCodexFiveHourUsage = async (): Promise<void> => {
          const model = activeSessionModel ?? (kiloConfig?.config.model as string | undefined) ?? "";
          if (!model.startsWith("openai/")) {
            codexFiveHourRemaining = null;
            return;
          }
          try {
            const used = await fetchKiloCodexFiveHourPercent();
            codexFiveHourRemaining = used === null ? null : remainingPercent(used);
          } catch {
            // Usage is supplemental; auth/network failures must not interrupt a turn.
            codexFiveHourRemaining = null;
          }
        };
        const formatCodexFiveHourUsage = (): string => codexFiveHourRemaining === null
          ? ""
          : `  5h ${colorRemainingPercent(codexFiveHourRemaining)} left`;
        const formatMainUsage = (used: number, costAmount: number): string =>
          `🔢 ${used} tokens used, cost $${costAmount.toFixed(6)}${formatCodexFiveHourUsage()}`;

        // Compact one-line summary for a session entry, used in the "previous"
        // subsection under each role. Rendered dim/grey. `cache` is the
        // session's cached-read token total from kilo.db (best-effort; hidden
        // when unknown/zero).
        const formatCompactRow = (id: string, u: { used: number; size: number; costAmount: number; costCurrency: string }, cache: number | null = null): string => {
          const ctx =
            u.size > 0
          ...
          const cacheBit = cache != null && cache > 0 ? `  cache ${cache.toLocaleString()}` : "";
          return `${C.dim}       ${id}  ctx ${ctx}  cost ${cost}${cacheBit}${C.reset}`;
        };

        const printStatus = (): void => {
          // Prefer the model the active session actually uses (reported by
          // `session/load` on resume); fall back to the config `model` for
          // fresh sessions or when the agent didn't expose config options.
          const mainModel = activeSessionModel ?? (kiloConfig?.config.model as string | undefined) ?? "(kilo default)";
          const mainId = activeSession?.sessionId ?? null;
          const mainName = mainId ? (sessionNames.get(mainId) ?? null) : null;
          // Per-session cached-read token totals from kilo.db (best-effort).
          // The ACP usage_update only reports used = input + cache.read, so
          // the cache split is supplemented from the local message history.
          const cacheIds = new Set<string>(sessionUsage.keys());
          if (mainId) cacheIds.add(mainId);
          const sumIdNow = thinkSummarizer?.sessionId ?? null;
          if (sumIdNow) cacheIds.add(sumIdNow);
          const cacheBySession = readKiloCachedTokens(Array.from(cacheIds));
          const sessionCache = (id: string | null): number | null => {
            if (!id) return null;
            const v = cacheBySession.get(id);
            return v == null ? null : v;
          };

          // Main session block (bold header). Lists the current main session's
          // name/model/context/cost, then any prior main sessions (compacted
          // away) as compact rows — excluding the current one, and only when
          // there is more than one main session.
          console.log(`\n  ${C.bold}main${C.reset}`);
          if (mainName) {
          ...
          // The thinking/effort level the session actually uses, shown only
          // when the active model exposes effort levels (i.e. it has thinking
          // control). `currentValue` is already resolved by the agent from the
          // saved/default/first variant, so this is the real level in use —
          // never a bare unresolved "default". The display name applies the
          // openrouter naming rule (canonical name, alias in parens).
          if (sessionEffortOptions && sessionEffortOptions.length > 0) {
            console.log(`     thinking: ${formatEffortDisplay(activeEffort, sessionEffortOptions)}`);
          }
          if (mainUsage.size > 0) {
          ...
          console.log(`     cost    : ${formatCost(mainUsage.costAmount, mainUsage.costCurrency)} (cumulative)${formatCodexFiveHourUsage()}`);
          const mainCache = sessionCache(mainId);
          if (mainCache != null && mainCache > 0) {
            console.log(`     cache  : ${mainCache.toLocaleString()} tokens (cached reads)`);
          }
          const prevMain = Array.from(sessionUsage.entries()).filter(
            ([id, u]) => u.role === "main" && id !== mainId,
          );
          if (prevMain.length > 0) {
            console.log(`     ${C.dim}previous:${C.reset}`);
            for (const [id, u] of prevMain) console.log(formatCompactRow(id, u, sessionCache(id)));
          }

          // Summarizer block — entire block rendered grey. No "state:" field;
          // current summarizer session plus any prior ones (restarted on
          // compact) as compact rows.
          const sumId = thinkSummarizer?.sessionId ?? null;
          const sumUsage = thinkSummarizer?.lastUsage ?? null;
          console.log(`  ${C.dim}summarizer${C.reset}`);
          console.log(`     ${C.dim}session : ${sumId ?? "(none)"}${C.reset}`);
          console.log(`     ${C.dim}model   : ${smallModel}${C.reset}`);
          if (sumUsage && (sumUsage.used > 0 || sumUsage.size > 0)) {
            if (sumUsage.size > 0) {
              console.log(`     ${C.dim}context : ${sumUsage.used.toLocaleString()} / ${sumUsage.size.toLocaleString()} tokens${C.reset}`);
            } else if (sumUsage.used > 0) {
              console.log(`     ${C.dim}context : ${sumUsage.used.toLocaleString()} tokens${C.reset}`);
            }
          }
          if (sumUsage && sumUsage.costAmount > 0) {
            console.log(`     ${C.dim}cost    : ${formatCost(sumUsage.costAmount, sumUsage.costCurrency)} (cumulative)${C.reset}`);
          }
          const sumCache = sessionCache(sumId);
          if (sumCache != null && sumCache > 0) {
            console.log(`     ${C.dim}cache  : ${sumCache.toLocaleString()} tokens (cached reads)${C.reset}`);
          }
          const prevSum = Array.from(sessionUsage.entries()).filter(
            ([id, u]) => u.role === "summarizer" && id !== sumId,
          );
          if (prevSum.length > 0) {
            console.log(`     ${C.dim}previous:${C.reset}`);
            for (const [id, u] of prevSum) console.log(formatCompactRow(id, u, sessionCache(id)));
          }
          console.log();
        };
        ...
        // the resumed session.
        if (activeSession) {
          const title = await fetchSessionTitle(ctx, activeSession.sessionId);
          if (title) sessionNames.set(activeSession.sessionId, title);
        }
        await refreshCodexFiveHourUsage();
        printStatus();
        ...
        while (true) {
          ...
          const slashCommand = parseSlashCommandInvocation(promptText);
          if (
            slashCommand &&
            slashCommand.args &&
            NO_ARG_COMMANDS.has(slashCommand.command)
          ) {
            console.error(`\n⚠️  "${slashCommand.command}" does not accept arguments.\n`);
            continue;
          }
          ...
          if (trimmed === "/compact") {
            await handleCompact(ctx, () => session, (s) => {
              session = s;
              activeSession = s;
              // activeSessionModel and sessionModelOptions are refreshed by
              // handleCompact from the fresh session's response.
          ...
          if (trimmed === "/thinking") {
            await handleThinking(ctx);
            continue;
          }

          if (trimmed === "/codex-balance") {
            try {
          ...
          if (trimmed === "/kilo-gateway-balance") {
            try {
              const out = await handleKiloBalance();
              console.log(`\n${out}\n`);
            } catch (err) {
              console.error(`\n⚠️  ${(err as Error).message}\n`);
            }
          ...
          if (trimmed === "/sessions" || slashCommand?.command === "/sessions") {
            // Optional integer arg = page index; offset into the newest-first
            // session list is arg * 10 (see KILO_PAGE_SIZE in kiloBalance.js).
            let page = 0;
            if (slashCommand && slashCommand.args) {
              const m = slashCommand.args.trim().match(/\d+/);
              if (m) page = Math.max(0, Number(m[0]));
            }
            try {
              const out = handleKiloSessions(page);
              console.log(`\n${out}\n`);
            } catch (err) {
              console.error(`\n⚠️  ${(err as Error).message}\n`);
            }
          ...
          if (trimmed === "/kilo-gateway-logs" || slashCommand?.command === "/kilo-gateway-logs") {
            // Optional integer arg = page index; offset into the newest-first
            // request list is arg * 10 (see KILO_PAGE_SIZE in kiloBalance.js).
            let page = 0;
            if (slashCommand && slashCommand.args) {
              const m = slashCommand.args.trim().match(/\d+/);
              if (m) page = Math.max(0, Number(m[0]));
            ...
            try {
              const out = handleKiloGatewayRequests(page);
              console.log(`\n${out}\n`);
            } catch (err) {
              console.error(`\n⚠️  ${(err as Error).message}\n`);
            }
          ...
          lastRenderedToolCallId = null;
          lastStatus.clear();
          renderedInputSummaryParts.clear();
          ...
          const deferredOutputs: Array<string | (() => string)> = [];
          summaryGate?.reset();
          ...
          const processUpdate = async (message: SessionUpdateKind): Promise<boolean> => {
            ...
            switch (update.sessionUpdate) {
              ...
              case "usage_update":
                ...
                deferredOutputs.push(
                  () => formatMainUsage(update.used, update.cost?.amount ?? 0),
                );
                break;
              default:
                endThinking();
            ...
            if (cancelController.signal.aborted) {
              ...
              while (true) {
                ...
                if (upd.sessionUpdate === "usage_update") {
                  ...
                  deferredOutputs.push(
                    () => formatMainUsage(upd.used, upd.cost?.amount ?? 0),
                  );
                }
              }
              console.log("\nCanceled.");
          ...
          if (usedTokens > 0) await refreshCodexFiveHourUsage();
          if (deferredOutputs.length > 0) {
            for (const out of deferredOutputs) {
              console.log(`\n${typeof out === "function" ? out() : out}`);
            }
          }
