...
const COMMAND_HELP: Record<string, string> = {
  ...
  "/codex-usage": "Show Codex (ChatGPT subscription) rate-limit usage",
...
const IMPLEMENTED_COMMANDS = new Set(["/exit", "/quit", "/help", "/status", "/compact", "/model", "/models", "/thinking", "/codex-usage"]);
const STUB_COMMANDS = new Set(
  COMMANDS.filter((c) => !IMPLEMENTED_COMMANDS.has(c)),
);
const NO_ARG_COMMANDS = new Set([...COMMANDS, "/h", "/?"]);
  ...
  try {
    await acp
      ...
      .connectWith(stream, async (ctx) => {
        ...
        // subsection under each role. Rendered dim/grey.
        const formatCompactRow = (id: string, u: { used: number; size: number; costAmount: number; costCurrency: string }): string => {
          const ctx =
            u.size > 0
          ...
          return `${C.dim}       ${id}  ctx ${ctx}  cost ${cost}${C.reset}`;
        };
        ...
        const printStatus = (): void => {
          ...
          if (prevMain.length > 0) {
            ...
            for (const [id, u] of prevMain) console.log(formatCompactRow(id, u));
          }
          ...
          if (prevSum.length > 0) {
            console.log(`     ${C.dim}previous:${C.reset}`);
            for (const [id, u] of prevSum) console.log(formatCompactRow(id, u));
          }
          console.log();
        };
