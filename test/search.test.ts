...
describe("reverse-i-search (ctrl-r)", () => {
  ...
  it("ctrl-right (CSI 1;5C) accepts the search like Enter, not cancels it", async () => {
    const stdout = makeStdout();
    const ri = new RawInput(stdout as unknown as any, {
      prompt: "> ",
      history: [H("hello world"), H("goodbye")],
    ...
    ri.handleBytes(Buffer.from([0x12])); // ctrl-r
    ri.handleBytes(Buffer.from("hello")); // matches "hello world"
    // Ctrl+Right is sent as ESC [ 1 ; 5 C; it must accept, not cancel.
    ri.handleBytes(Buffer.from([0x1b, 0x5b, 0x31, 0x3b, 0x35, 0x43]));
    assert.ok(!ri.isSearching, "search should be inactive after ctrl-right accept");
    ri.handleBytes(Buffer.from([0x0d])); // Enter → send the accepted text
    assert.strictEqual(await readPromise, "hello world");
  });

  it("ctrl-left (CSI 1;5D) accepts the search like Enter, not cancels it", async () => {
    const stdout = makeStdout();
    const ri = new RawInput(stdout as unknown as any, {
      prompt: "> ",
      history: [H("hello world"), H("goodbye")],
    ...
    ri.handleBytes(Buffer.from([0x12])); // ctrl-r
    ri.handleBytes(Buffer.from("hello")); // matches "hello world"
    // Ctrl+Left is sent as ESC [ 1 ; 5 D; it must accept, not cancel.
    ri.handleBytes(Buffer.from([0x1b, 0x5b, 0x31, 0x3b, 0x35, 0x44]));
    assert.ok(!ri.isSearching, "search should be inactive after ctrl-left accept");
    ri.handleBytes(Buffer.from([0x0d])); // Enter → send the accepted text
    assert.strictEqual(await readPromise, "hello world");
  });
