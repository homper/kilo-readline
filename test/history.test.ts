...
test("updateRecentList moves the value to the front and dedupes", () => {
  assert.deepEqual(updateRecentList(["a", "b", "c"], "a", 5), ["a", "b", "c"]);
  assert.deepEqual(updateRecentList(["a", "b", "c"], "b", 5), ["b", "a", "c"]);
  assert.deepEqual(updateRecentList(["a", "b"], "c", 5), ["c", "a", "b"]);
});

test("updateRecentList caps to the given length (keeping most-recent first)", () => {
  // unshift("e") -> ["e","a","b","c","d"], then cap to 3 keeps ["e","a","b"].
  assert.deepEqual(updateRecentList(["a", "b", "c", "d"], "e", 3), [
    "e",
    "a",
    "b",
  ]);
  // Re-selecting an existing entry when already at cap must not grow the list.
  assert.deepEqual(updateRecentList(["a", "b", "c"], "c", 3), [
    "c",
    "a",
    "b",
  ]);
...
test("updateRecentList ignores empty/whitespace-only values", () => {
  assert.deepEqual(updateRecentList(["a", "b"], "  ", 5), ["a", "b"]);
  assert.deepEqual(updateRecentList(["a", "b"], "", 5), ["a", "b"]);
});
